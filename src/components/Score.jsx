...
const Score = ({scoreOn, setScoreOn}) => {
 ...
 async function fetchData(){
  ...
  setWinners(response.data.slice(3))
        ...
        >
          ...
          <div className="p-8 w-72 ml-2 bg-theme_yellow font-poppins rounded-3xl flex flex-col">
            ...
            <ul className="text-xl font-semibold">
              {winners.map((winner,index)=>(<li key={index} className="flex items-center gap-3">
              
                {winner}
                {' '}
                <span className=" text-2xl">
                 {winner>0 ? <AnimatedCounter endValue={winner.Point} duration={1000} />:0}
                </span>
              </li>))}
